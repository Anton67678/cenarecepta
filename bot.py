...
def format_recipe(recipe):
    ...
    # Защита от нуля — сайт мог сохранить 0 при незаполненном поле
    yield_count = recipe.get('yield_count', 1) or 1
    ...
    # ── Ингредиенты ──────────────────────────────────────────────────────
    # Поле 'recipe' хранит количество на ВЕСЬ замес → делим на yield_count
    ...
    if ingredients:
        ...
        for ing in ingredients:
            ...
            amount_batch = ing.get('recipe', 0)           # г на весь замес
            amount_per_unit = amount_batch / yield_count  # г на 1 шт
            cost_ing = (price_pkg / pkg) * amount_per_unit
            ing_total += cost_ing
            if n:
                text += (
                    f'  • {n}: {amount_per_unit:.1f}г/шт'
                    f' — {cost_ing:.2f} ₽/шт\n'
                )
        text += f'  <i>Итого ингредиенты: {ing_total:.2f} ₽/шт</i>\n'

    # ── Расходники ────────────────────────────────────────────────────────
    # Поле 'per' хранит количество на ВЕСЬ замес → тоже делим на yield_count
    ...
    if consumables:
        ...
        for c in consumables:
            ...
            per_batch = c.get('per', 0)                   # шт на весь замес
            per_unit = per_batch / yield_count            # шт на 1 шт
            cost_c = (price_pkg / pkg) * per_unit         # ← ИСПРАВЛЕНО
            cons_total += cost_c
            if n:
                text += (
                    f'  • {n}: {per_unit:.2f} шт/шт'
                    f' — {cost_c:.2f} ₽/шт\n'
                )
        text += f'  <i>Итого расходники: {cons_total:.2f} ₽/шт</i>\n'

    # ── Декор и упаковка ──────────────────────────────────────────────────
    # Аналогично расходникам — per хранит кол-во на замес
    ...
    if decor:
        ...
        for d in decor:
            ...
            per_batch = d.get('per', 0)
            per_unit = per_batch / yield_count
            cost_d = (price_pkg / pkg) * per_unit         # ← ИСПРАВЛЕНО
            decor_total += cost_d
            if n:
                text += (
                    f'  • {n}: {per_unit:.2f} шт/шт'
        ...
        text += f'  <i>Итого декор: {decor_total:.2f} ₽/шт</i>\n'

    text += '\n' + '─' * 22 + '\n'
    text += f'💰 <b>Себестоимость: {cost:.2f} ₽/шт</b>\n'

    # Наценка: если сайт хранит markup_pct как процент сверх себестоимости
    # (100 → цена = себестоимость × 2.0), формула ×(1 + markup/100) верна.
...
async def cmd_recipes(message: types.Message):
    ...
    if not recipes:
        await message.answer(
            '📋 У тебя пока нет сохранённых рецептур.\n\n'
